...
export default function DocumentStorage({ route }) {
  ...
  useEffect(() => {
    // 새로운 처방전이 추가될 때마다 기존 리스트에 추가
    if (route.params?.newPrescription) {
      setPrescriptions(prevPrescriptions => [
        route.params.newPrescription,  // 새로운 처방전을 맨 앞에 추가
        ...prevPrescriptions  // 기존 처방전 유지
      ]);
    }
  }, [route.params?.newPrescription]);  // route.params.newPrescription이 변경될 때마다 실행
