...
export default function DocumentStorage({ route }) {
  ...
  const fetchPrescriptions = async () => {
    try {
      const response = await axios.get(
        getApiUrl(ENDPOINTS.prescriptionList),
        {
          headers: {
  ...
  const fetchPrescriptionsByDate = async () => {
    try {
      const response = await axios.get(
        getApiUrl(ENDPOINTS.prescriptionsByDate),
        {
          headers: {
            Authorization: `Token ${userToken}`,
            "Content-Type": "application/json"
    ...
    >
      <View style={styles.itemContent}>
        ...
        <Text style={styles.duration}>
          {item.duration ? `${item.duration}일 | ${item.endDate} 까지` : item.endDate}
        </Text>
      </View>
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.container}>
        ...
        <View style={styles.subHeader}>
          ...
          <TouchableOpacity
            style={[styles.filterButton, sortByDate && styles.filterButtonActive]}
            onPress={handleSort}
          >
            <Text style={[styles.filterText, sortByDate && styles.filterTextActive]}>
              {sortByDate ? "기본순" : "최신순"}
            </Text>
            <MaterialIcons
